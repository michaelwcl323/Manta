...
def _host_override_map(wan_cfg: dict | None) -> dict[str, dict]:
    if not isinstance(wan_cfg, dict):
        return {}
    raw_hosts = wan_cfg.get('hosts') or []
    if not isinstance(raw_hosts, list):
        raise BenchError('cloudlab_wan.hosts must be a list', TypeError(type(raw_hosts)))

    overrides: dict[str, dict] = {}
    for raw in raw_hosts:
        if not isinstance(raw, dict):
            raise BenchError('cloudlab_wan.hosts entries must be objects', TypeError(type(raw)))
        hostname = raw.get('hostname')
        if not hostname or not isinstance(hostname, str):
            raise BenchError('cloudlab_wan.hosts entries must include hostname', KeyError('hostname'))
        key = hostname.split(':')[0].strip()
        overrides[key] = raw
    return overrides


def _merge_host_info(host_info: list[dict], wan_cfg: dict | None) -> list[dict]:
    overrides = _host_override_map(wan_cfg)
    merged = []
...
def _site_for_host(host: dict, octet2_site: dict[str, str]) -> str:
    host_override = host.get('wan_site')
    if host_override:
        return _normalize_site(host_override)

    region = host.get('region')
    if isinstance(region, str) and region.strip() and region.strip().lower() != 'default':
        return _normalize_site(region)

    return _site_for_ip(_host_ipv4(host['hostname']), octet2_site, None)
...
def _build_plan(
    ...
    for h in host_info:
        ...
        site = _site_for_host(h, octet2_site)
