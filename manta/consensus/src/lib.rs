...
impl Consensus {
    ...
    fn flexible_commit_candidate(&self, support_round: Round) -> Option<(Round, Round)> {
        ...
        if trigger_base % wave_length != 0 {
            return None;
        }
        if trigger_base < 2 * wave_length {
            return None;
        }
    ...
    async fn try_commit(
        ...
        //
        // As in project, a single support check decides whether we can commit this leader.
...
leader_digest(cert)= {:?} -> {:?} (node_id={})",
    ...
    /// Order leader certificates to commit, mirroring project's `order_leaders` with step
